...
ApiServer::ApiServer(int port, PlayerManager* pPlayerManager, QObject* parent)
        : QObject(parent),
          m_port(port),
...
void ApiServer::start() {
    ...
    moveToThread(thread);
    thread->start();
    qInfo() << kApiTag << "Starting on port" << m_port;
}
...
void ApiServer::run() {
    ...
    // Detailed deck info — track metadata, analysis results
    ...
    // ════════════════════════════════════════════════════════════════
    // GENERIC CONTROL (read/write any Mixxx control)
    // ════════════════════════════════════════════════════════════════
    ...
    svr.Get("/api/control", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        res.set_content(toJson(QJsonObject{
                {"group", group}, {"key", key}, {"value", getControl(group, key)}
        }), "application/json");
    });

    svr.Post("/api/control", [this](const httplib::Request& req, httplib::Response& res) {
        auto obj = parseBody(req.body);
        setControl(obj["group"].toString(), obj["key"].toString(), obj["value"].toDouble());
    ...
    // Load track by file path — THE key API for programmatic DJ control
    svr.Post("/api/load", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (m_pPlayerManager && !path.isEmpty()) {
            // Must dispatch to main thread — PlayerManager is not thread-safe
            QMetaObject::invokeMethod(m_pPlayerManager,
                    [pm = m_pPlayerManager, path, deck]() {
                        pm->slotLoadToDeck(path, deck);
                    },
                    Qt::QueuedConnection);
            qInfo() << kApiTag << "Loading" << path << "to deck" << deck;
            // Small delay to let the main thread process
            QThread::msleep(500);
            res.set_content(toJson(jsonOk({{"deck", deck}, {"track", path}})), "application/json");
        } else {
            res.set_content(toJson(jsonError("PlayerManager not available or empty path")), "application/json");
        }
    });
    ...
    svr.Post("/api/seek", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        res.set_content(toJson(jsonOk({{"deck", deck}, {"position", position}})), "application/json");
    });
    ...
    svr.Post("/api/crossfade", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        // Map 0-1 (our API) to -1 to +1 (Mixxx)
    ...
    svr.Post("/api/eq", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (obj.contains("hi"))  setControl(eqGroup, "parameter3", obj["hi"].toDouble());
        if (obj.contains("mid")) setControl(eqGroup, "parameter2", obj["mid"].toDouble());
        if (obj.contains("lo"))  setControl(eqGroup, "parameter1", obj["lo"].toDouble());
        res.set_content(toJson(jsonOk()), "application/json");
    });
    ...
    svr.Post("/api/loop", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (action == "in") setControl(g, "loop_in", 1.0);
        else if (action == "out") setControl(g, "loop_out", 1.0);
        else if (action == "toggle") setControl(g, "reloop_toggle", 1.0);
        else if (action == "halve") setControl(g, "loop_halve", 1.0);
        else if (action == "double") setControl(g, "loop_double", 1.0);
    ...
    svr.Post("/api/library", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (action == "down") setControl("[Library]", "MoveDown", 1.0);
        else if (action == "up") setControl("[Library]", "MoveUp", 1.0);
        else if (action == "left") setControl("[Library]", "MoveLeft", 1.0);
        else if (action == "right") setControl("[Library]", "MoveRight", 1.0);
        else if (action == "focus_forward") setControl("[Library]", "MoveFocusForward", 1.0);
        else if (action == "focus_back") setControl("[Library]", "MoveFocusBackward", 1.0);
        else if (action == "select") setControl("[Library]", "GoToItem", 1.0);
        res.set_content(toJson(jsonOk({{"action", action}})), "application/json");
    });
    ...
    svr.Post("/api/effect", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        QString action = obj["action"].toString("toggle"); // toggle, on, off
        ...
        if (obj.contains("mix")) setControl(g, "mix", obj["mix"].toDouble());
        if (obj.contains("super")) setControl(g, "super1", obj["super"].toDouble());
    ...
    // ════════════════════════════════════════════════════════════════
    // SMOOTH TRANSITION (server-side, non-blocking)
    // ════════════════════════════════════════════════════════════════
    ...
    svr.Post("/api/transition", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        std::thread([this, toDeck, duration]() {
            ...
            for (int i = 0; i <= steps; i++) {
                ...
                double ease = r * r * (3.0 - 2.0 * r); // S-curve
                double cf = cfStart + (cfEnd - cfStart) * ease;
                setControl("[Master]", "crossfader", cf);
                std::this_thread::sleep_for(std::chrono::milliseconds(1000 / fps));
            }
            setControl("[Master]", "crossfader", cfEnd);
            qInfo() << kApiTag << "Transition to deck" << toDeck << "complete (" << duration << "s)";
        }).detach();

        res.set_content(toJson(jsonOk({
                {"deck", toDeck},
                {"duration", duration},
    ...
    svr.Post("/api/filter", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        setControl(QString("[QuickEffectRack1_[Channel%1]]").arg(deck), "super1", value);
        res.set_content(toJson(jsonOk()), "application/json");
    });
    ...
    // ════════════════════════════════════════════════════════════════
    // LIST ALL TRACKS (files in ~/Music)
    // ════════════════════════════════════════════════════════════════
    ...
    svr.Get("/api/tracks", [](const httplib::Request&, httplib::Response& res) {
        ...
        res.set_content(QJsonDocument(result).toJson(QJsonDocument::Compact).toStdString(), "application/json");
    });

    // ════════════════════════════════════════════════════════════════

    qInfo() << kApiTag << "HTTP API listening on port" << m_port;
